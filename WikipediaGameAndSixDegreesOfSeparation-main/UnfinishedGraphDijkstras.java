...
public class UnfinishedGraphDijkstras<T> implements GraphADT<T>, IUnfinishedGraphDijkstras<T> {
    ...
    private ArrayList<Path> paths = new ArrayList<>(); // holds paths
    public UnfinishedGraphDijkstras() {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            this.dataSequence.add(extendBy.target.data);//adds the new vertex target to dataSequence.
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        if(start == null || end == null) {//if either start or end are null throw exception
            throw new NoSuchElementException("start or end is null");
        }
        if(start.equals(end) || start == end) {//if the start and end are equal, short path found.
        ...
        PriorityQueue queue = new PriorityQueue<Path>();//PQ to represent the Frontier

        Vertex curNode = vertices.get(start);
        Path p = new Path(curNode);
        queue.add(p);
        Hashtable<T, Boolean> visitedHash = new Hashtable<>();//hash table to track visited values, alligns with graph
        //Using a hash table here will work cause all values will be false by default
        Hashtable<T, Integer> distanceHash = new Hashtable<>();

        Path shortPath = null;//shortest path that will be returned

        while(!queue.isEmpty()) {

            Path curPath = (UnfinishedGraphDijkstras.Path) queue.remove();

            if(shortPath != null && shortPath.distance <= curPath.distance) {//if the current path is shorter than the shortest path
                return shortPath;//return the shortest path
            }
            if(curPath.distance > 2) {//if the current path is longer than 2 or less than 0
                return null;
            }
            visitedHash.remove(curPath.end.data);
            visitedHash.put(curPath.end.data, true);//making the value visited true
            distanceHash.remove(curPath.end.data);
            distanceHash.put(curPath.end.data, curPath.distance);
            List<Edge> edges = new LinkedList<>(curPath.end.edgesLeaving);

            //Selection Sort to put all the edges in order from smallest to greatest weight
            //this will help the alg run faster as they are being sorted by weight so it will
            //pick the most effective path.
            for(int i = 0; i < edges.size(); i++) {
                int indexSmallest = i;
                for(int j = i + 1; j < edges.size(); j++) {
                    if(edges.get(j).weight < edges.get(indexSmallest).weight) {
                        indexSmallest = j;
                    }
                }
                Edge temp = edges.get(i);
                edges.set(i, edges.get(indexSmallest));
                edges.set(indexSmallest, temp);
            }

            //This for loop will go through all the edges for the given node, check if they are visited or not
            //and see if the final path has been reacched.
            for(Edge curEdge : edges) {//looks through all the edges from the current Node in the queue
                Path newPath = new Path(curPath, curEdge);//new path to look at

                if(visitedHash.containsKey(curEdge.target.data)) {//checks to see if the node that is being looked for exists
                    if(!visitedHash.get(curEdge.target.data)) {//Checks if the node has been visited
                        queue.add(newPath);
                    }
                ...
                if(curEdge.target.data.toString().toLowerCase(Locale.ROOT).equals(end.toString().toLowerCase(Locale.ROOT))) { //checks to see if the end was reached

                    if(shortPath == null) {
                        shortPath = newPath;
                        paths.add(shortPath);
                    }
                    else if(newPath.compareTo(shortPath) <= 0) {//the two paths are identical in length,
                        shortPath = newPath;
                        paths.add(newPath);//adds the path to the list of paths
                    }
                }
            }
        }

        if(shortPath == null) {//if the shortest path is null, no path was found and exception should be thrown
            throw new NoSuchElementException("null path");
        }
    ...
    public ArrayList<String> allShortestPaths(T start, T end) {
        paths.clear();
        dijkstrasShortestPath(start, end);

        ArrayList<String> allPaths = new ArrayList<>();
        paths.forEach(e -> allPaths.add(String.valueOf(e.dataSequence)));
        System.out.println("got all paths leading to " + end);
        return allPaths;
    }
}
